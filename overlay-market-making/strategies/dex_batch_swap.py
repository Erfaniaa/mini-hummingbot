...
class DexBatchSwap:
    ...
    def __init__(self, cfg: DexBatchSwapConfig) -> None:
        ...
        # Remaining amounts per level (in the chosen basis)
    ...
    def _current_price(self) -> Optional[float]:
        # Use first connector for pricing (same across wallets)
        try:
    ...
    def _should_execute(self, price: float, level: float) -> bool:
        if self.cfg.amount_is_base:
            # Spending base to receive quote -> prefer higher quote_per_base
            return price >= level
        else:
            # Spending quote to receive base -> prefer lower quote_per_base
            return price <= level

    def _execute_level(self, li: int, amount: float) -> None:
        # Execute across all wallets
        for conn in self.connectors:
            try:
                ...
                # Leave amount remaining for retry on next ticks
                return
        # If all wallets succeeded, mark level done
        self.done[li] = True
    ...
    def _on_tick(self) -> None:
        ...
        for i, (lvl, amt, done) in enumerate(zip(self.levels, self.remaining, self.done)):
            ...
            if self._should_execute(price, lvl):
                # Clamp by available balance before submitting (per wallet checked internally)
        ...
        # Stop if all levels executed
        if all(self.done):
            self.stop()

    def _on_error(self, e: Exception) -> None:
        # Keep running; print compact error message
